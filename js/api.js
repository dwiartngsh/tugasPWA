      ...
      .then(function(data) {
        ...
        <div class="row">
          ...
          teamHTML += `
                      ...
                      `;
        document.getElementById("body-content").innerHTML = teamHTML;
        //kirim objek data hasil parsing json agar bisa disimpan ke index db
        resolve(data);
      });
  })
}
...
function getSavedTeams() {
  getAll().then(function(data) {
    console.log(data);
    //menyusun komponen card artikel secara dimanis
    var teamsHTML = "";
        ...
        <div class="col s12">
          <div class="card">
            <div class="card-image">
              ...
              <button class="btn-floating halfway-fab waves-effect waves-light red" id="delete" value="${data.id}">
                <i class="material-icons">detele</i>
              </button>
            </div>
            <div class="card-content">
              <span clas="card-title" align="center" style="font-weight:bold; margin-bottom:20px; color:#0D47A1;">
                <u>${data.name}</u>
              </span>
              <p align="center">Nickname: ${data.shortName}<br> Address : ${data.address}<br>
              Founded: ${data.founded}<br> Club Colors: ${data.clubColors}<br> Venue: ${data.venue}</p>
            </div>
          </div>
        </div>
      </div>
...
//sisipkan komponen card ke dalam elemen dengan id #body-content
    ...
    for(let button of btn) {
      button.addEventListener("click", function (event) {
        ...
        console.log(id);
        var toastHTML = `<span>Successfully remove the team from favorite</span>
        <button onclick="M.Toast.getInstance(this.parentElement).dismiss()" class="btn-flat toast-action">Close</button>`;
